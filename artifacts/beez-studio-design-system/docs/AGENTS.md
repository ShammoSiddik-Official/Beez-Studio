# BeeZ Studio Design System design system
...
workspace package (`@workspace/beez-studio-design-system`): other artifacts depend
...
- `docs/references/README.md` — source provenance and retained BeeZ Studio brand
  references.
- `docs/references/component-inventory.md` — extracted reusable component
  families and their implementation status.
- `docs/references/components/` — source evidence and usage guidance for the
  extracted component families.
- `docs/references/logos/beez-studio-logo.png` — the retained primary BeeZ Studio
  mark used by the preview.
...
`@workspace/beez-studio-design-system/components/...`. Never use a `@/` alias inside
...
## Extracted BeeZ Studio visual language

- Use warm architectural off-white surfaces and near-black text as the default.
- Use the extracted BeeZ lime green (`#6f962c` in light mode and `#8ec431` in
  dark mode) for primary action, focus, active, and selected states.
- Use `Inter` for UI and body copy; use `Playfair Display` for editorial display
  headings.
- Keep the base radius and spacing step compact at `0.25rem`; derive larger
  treatments from that base instead of introducing arbitrary values.
- Prefer thin borders, restrained shadows, generous whitespace, and strong
  imagery over decorative effects.
- Keep motion measured: smooth scroll, small reveal transitions, and reduced
  motion support.
- Keep public-site and Console compositions separate while sharing these
  product-agnostic foundations.
...
for `@workspace/beez-studio-design-system` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/beez-studio-design-system` as a `workspace:*` dependency,
...
- Building or styling web UI: `artifacts/beez-studio-design-system/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/beez-studio-design-system/docs/consuming-expo.md`
- Building or styling a slide deck: `artifacts/beez-studio-design-system/docs/consuming-slides.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/beez-studio-design-system/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/beez-studio-design-system/docs/migrating-expo.md`

    ...
    <div className="space-y-4">
      <section className="rounded-xl border bg-card p-5 text-card-foreground">
        ...
        <h2 className="mt-3 font-serif text-3xl font-semibold tracking-tight">
          Quiet structure. Clear intent.
        </h2>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
          BeeZ Studio pairs editorial typography and generous neutral space with a
          lime-green signal for action, selection, and progress.
        </p>
        <div className="mt-5">
          <Guidelines
            items={[
              { kind: 'do', text: 'Use lime green sparingly for decisions and calls to action.' },
              { kind: 'do', text: 'Let imagery, typography, and whitespace carry the visual hierarchy.' },
              { kind: 'dont', text: 'Do not turn every surface into a high-saturation brand block.' },
            ]}
          />
        </div>
      </section>
  ...
  return (
    <div className="space-y-8 rounded-xl border bg-card p-6 text-card-foreground">
      <section>
        ...
        <p className="mt-4 font-serif text-4xl font-semibold">The quick brown fox</p>
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border bg-card p-6 text-card-foreground">
        ...
        <div className="mt-5 flex min-h-32 items-center justify-center rounded-lg bg-foreground p-8">
          <img
            src={`${import.meta.env.BASE_URL}beez-studio-logo.png`}
            alt="BeeZ Studio"
            className="max-h-16 w-auto object-contain"
          />
        </div>
        <p className="mt-4 text-sm text-muted-foreground">
          Use the supplied BeeZ Studio mark as the identity anchor. Keep it clear,
          unwarped, and surrounded by generous space.
        </p>
      </section>
      <section className="rounded-xl border bg-card p-6 text-card-foreground">
        <h2 className="font-serif text-2xl font-semibold">Visual language</h2>
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Warm off-white canvases, near-black text, lime accents, and editorial
          serif display type create a calm, confident architectural tone.
        </p>
        <div className="mt-5">
          <Guidelines
            items={[
              { kind: 'do', text: 'Pair Playfair Display with Inter for an editorial but usable hierarchy.' },
              { kind: 'do', text: 'Use architectural imagery with strong crops and intentional negative space.' },
              { kind: 'dont', text: 'Do not introduce a second accent palette without a product reason.' },
            ]}
          />
        </div>
      </section>
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border bg-card p-6 text-card-foreground">
        ...
        <h2 className="mt-3 font-serif text-3xl font-semibold">Confident, precise, human.</h2>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
          Copy should explain architectural value clearly, avoid inflated claims,
          and make the next action obvious for a prospective client or studio team member.
        </p>
      </section>
      <section className="rounded-xl border bg-card p-6 text-card-foreground">
        ...
        <div className="mt-4">
          <Guidelines
            items={[
              { kind: 'do', text: 'Lead with a clear statement, then support it with evidence or a project detail.' },
              { kind: 'do', text: 'Keep labels short and use sentence case for operational UI.' },
              { kind: 'dont', text: 'Do not bury contact actions below decorative copy.' },
            ]}
          />
        </div>
      </section>
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border bg-card p-6 text-card-foreground">
        ...
        <h2 className="mt-3 font-serif text-3xl font-semibold">Measured movement.</h2>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
          Motion should reveal hierarchy and make navigation feel smooth, never compete
          with the work being shown.
        </p>
        <div className="mt-5">
          <Guidelines
            items={[
              { kind: 'do', text: 'Prefer soft fades, small vertical reveals, and smooth page scrolling.' },
              { kind: 'do', text: 'Respect reduced-motion preferences for all decorative transitions.' },
              { kind: 'dont', text: 'Do not use looping motion or large parallax effects on operational surfaces.' },
            ]}
          />
        </div>
      </section>
